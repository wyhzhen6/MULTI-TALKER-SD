...
def simulate(room,simulate_config,filepath):
    ...
    if room.is_compute_SRR:
        for i in range(len(audio)):
            ...
            rir2 = room.room.rir[len(room.room.rir)-1][i]
            room.SRR_circle.append(room.compute_SRR(audio[i],rir1,room.fs))
            room.SRR_linear.append(room.compute_SRR(audio[i],rir2,room.fs))
            if room.array_num == 2:
                rir3 = room.room.rir[room.mic_num+1][i]
                room.SRR_circle2.append(room.compute_SRR(audio[i],rir3,room.fs))
        ...
        if room.meeting_type == "speech":
            ...
            for index2,audio_host in enumerate(room.host_audio):
                ...
                rir_linear = room.room.rir[len(room.room.rir)-1][index2 + len(audio)]

                host_srr_circle.append(room.compute_SRR(audio_host,rir1,room.fs))
                host_srr_linear.append(room.compute_SRR(audio_host,rir_linear,room.fs))
                if room.array_num == 2:
                    rir2 = room.room.rir[room.mic_num+1][index2+len(audio)]
                    host_srr_circle2.append(room.compute_SRR(audio_host,rir2,room.fs))
            
            room.SRR_circle.append(np.mean(host_srr_circle))
            room.SRR_linear.append(np.mean(host_srr_linear))
            if room.array_num == 2:
                room.SRR_circle2.append(np.mean(host_srr_circle2))
        tmp =  np.mean(room.SRR_circle)         
        room.SRR.append(tmp.tolist()) 
        if room.array_num == 2:
            tmp =  np.mean(room.SRR_circle2)
            room.SRR.append(tmp.tolist())
        tmp =  np.mean(room.SRR_linear)
        room.SRR.append(tmp.tolist())
    ...
    #save clean audio
    arraynum = 0
    for i in range(len(room.mic_loc)):
        array_signal = room.room.mic_array.signals[arraynum : arraynum + len(room.mic_loc[i][1])]
        arraynum += len(room.mic_loc[i][1])
        
        output_path_clean = os.path.join(output_path,filename, f"array_{i+1}_clean_signal.wav")
        sf.write(
            output_path_clean,  
            array_signal.T,         
            room.fs,                   
        ...
        print(f"已保存第{i+1}个麦克风阵列的干净语音: {output_path_clean}")
...
def add_noise(room,category_files,target_categories,simulate_config,filepath):
    ...
    diffuse_noise = room.gen_diffuse_noise(os.path.join(diffuse_noise_path,diffuse_file))
    ...
    arraynum = 0
    for i in range(len(room.mic_loc)):
        clean_signal = room.room.mic_array.signals[arraynum : arraynum + len(room.mic_loc[i][1])]
        arraynum += len(room.mic_loc[i][1])
        final_signal = room._add_noise(speech_sig = torch.from_numpy(clean_signal.T),vad_duration = room.vad_dur,point_noise = torch.from_numpy(point_noise[i].T),diffuse_noise = torch.from_numpy(diffuse_noise[i].T), SNR_point = room.SNR_point,SNR_diffuse = room.SNR_diffuse)
        
        final_signal = final_signal.numpy()
        output_path_final = os.path.join(output_path,filename, f"array_{i+1}_final_signal.wav")
        sf.write(
            output_path_final,  
            final_signal,         
            room.fs,                   
        ...
        print(f"已保存第{i+1}个麦克风阵列的融合噪声语音: {output_path_final}")
    snr_avg = []
    for item in room.avg_snr:
        snr_avg.append(np.mean(item))
    room.avg_snr = snr_avg
    ...
    for filepath in os.listdir(list_path):
        if filepath.endswith('.list'):
            ...
            room = rir_room(
                ...
                array_num = simulate_config['array_num'],
                meeting_type = simulate_config['meeting_type'],        
                mic_num = simulate_config['mic_num'],
                #mic_center_pos = simulate_config['mic_center_pos'],
                mic_radius = simulate_config['mic_radius'],
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--config', type=str, default='/data/zjj/rir/rir_test/script/rir_simulate.json',
