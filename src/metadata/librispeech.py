...
def get_utterance(data_path, transcription_mapping):
    ...
    for wav in tqdm(wavs_generator, desc="Reading wav files", disable=True):
...
def get_transcription(data_path):
    ...
    trans_dict = {}
    for txt in tqdm(txt_generator, desc="Reading txt files", disable=True):
...
if __name__ == '__main__':
    ...
    logger.info(f"Starting metadata generation for LibriSpeech.")
    logger.info(f"Using librispeech_dir: {args.librispeech_dir}")
    logger.info(f"Output will be saved to: {args.output_dir}")
    logger.info(f"Looking for files with extension: {args.file_type}")
